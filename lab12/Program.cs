...
namespace lab12 {
    ...
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            //Проекция
            var w2 = writers.OrderByDescending(wr => wr.title).Select(wr => wr.title);
            //Сортировка
            var w3 = writers.OrderBy(wr => wr.surname);
            //Группировка по жанру
            var w4 = writers.GroupBy(wr => wr.genre);
            ...
            var w6 = writers.Take((writers.Count * 2 / 3));
            //Проверку на соответствие элементов списка условию согласно варианту задания.
            var w7 = writers.All(wr => wr.surname == "Толстой");
            var w8 = writers.Any(wr => wr.surname == "Толстой");
            ...
            var w9 = writers.Zip(secondSebsequence, (writers, age) => new {Writer = writers, Age = age});
